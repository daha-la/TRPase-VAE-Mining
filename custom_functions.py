...
class HMMmodule:
    ...
    def build(self):
        if not os.path.exists(f'hmm_model/{self.hmm_model}.hmm'):
            ...
            print(f'{self.hmmer_path}/hmmbuild --amino ../../hmm/{self.hmm_model}.hmm ../../alignments/muscle_outputs/{self.msa_name}.afa > ../../hmm/{self.hmm_model}_hmmbuild.log')
            subprocess.run(f'{self.hmmer_path}/hmmbuild --amino ../../hmm/{self.hmm_model}.hmm ../../alignments/muscle_outputs/{self.msa_name}.afa > ../../hmm/{self.hmm_model}_hmmbuild.log', shell=True, executable="/bin/zsh")

    def align(self,fasta_name,fasta_ext = 'faa',save_path = '../datasets'):
        subprocess.run(f'{self.hmmer_path}/hmmalign --trim --outformat afa ../../hmm/{self.hmm_model}.hmm ../datasets/{fasta_name}.{fasta_ext} > ../../alignments/{fasta_name}.afa', shell=True, executable="/bin/zsh")
        print(f'{self.hmmer_path}/hmmalign --trim --outformat afa ../../hmm/{self.hmm_model}.hmm ../datasets/{fasta_name}.{fasta_ext} > ../../alignments/{fasta_name}.afa')
